...
class seans_concavehull:
    ...
    def concavehull(self, k=None):
        ...
        # Per-node dynamic k: maps tuple(point) -> local k value for that node.
        node_k = {}
        ...
        while (currentPoint != firstPoint or step == 2) and self.numPoints > 0:
            ...
            # Find the k-nearest neighbors to the current point, using that node's local k.
            local_k = max(kk, node_k.get(tuple(currentPoint), kk))
            kNearestPoints = self.knn_query(dataSet, currentPoint, local_k)
            ...
            its = True
            i = 0
            earliest_intersect_idx = None  # Smallest hull index of intersecting edge (for candidate 0)
            ...
            while its == True and i < len(cPoints):
                ...
                if i == 0:
                    # Scan ALL edges to find the earliest (smallest hull index) intersection.
                    for jj in range(2, len(hull) - lastPoint + 1):
                        ...
                        if do_intersect(hull[-1], cPoints[0], segment_start, segment_end):
                            its = True
                            earliest_intersect_idx = seg_start_idx  # Largest jj → smallest index
                    if its:
                        i += 1
                else:
                    # Early-exit for subsequent candidates.
            ...
            if its:
                # All candidates intersect.
                # Trim hull back to the earliest intersecting vertex and increase k just for it.
                if earliest_intersect_idx is None or earliest_intersect_idx < 1:
                    # Can't trim safely; fall back to global restart.
                ...
                key = tuple(hull[earliest_intersect_idx])
                new_local_k = node_k.get(key, kk) + 1

                if new_local_k >= self.numPoints - 1:
                    # This vertex's k is exhausted; fall back to global restart.
                    if kk == self.numPoints - 1:
                        print("FAILED")
                        return None
                    self.numPoints = len(self.pointsList)
                ...
                # Restore trimmed points to dataset.
                for pt in hull[earliest_intersect_idx + 1:]:
                    dataSet = self.add_point(dataSet, pt)

                # Trim hull and rewind to the intersecting vertex.
                hull = hull[:earliest_intersect_idx + 1]
                currentPoint = hull[-1]
                node_k[key] = new_local_k
                kk += 1  # Increase global k on every trim.
                step = len(hull) + 1
                previousAngle = compute_angle(hull[-1], hull[-2]) if len(hull) >= 2 else 0
            else:
