...
| C-BASE-3 | On failure (dead end or excluded points) the original restarts from scratch with k+1, discarding all prior work. | established | [moreira2007] Alg. 1 lines 33, 45; `baseline/concavehull.py` recursive call | Runtime, Trimming | Motivates checkpoint trimming. |
...
| ID | Statement | Status | Evidence | Paper loc | Notes |
|----|-----------|--------|----------|-----------|-------|
| C-OPT-1 | Bucketing reduces k-NN to expected sub-linear cost. | conjectured | `knn_query` (grid expansion) in `legacy/monolithic/seans_concavehull_4.py`; [sean2025] §5 | Bucketing | **Not in the draft, and NOT in the experiment harness.** The harness's k-NN (`knn.py`) uses scipy `cKDTree` (tombstoned deletes + lazy rebuild), not a hand-rolled grid; only the *intersection* check is grid-bucketed (→ C-OPT-2). So the "grid bucketing for k-NN" idea currently has no live implementation backing it beyond the older `legacy/monolithic/seans_concavehull_4.py`. Keep distinct from C-OPT-2. **Inconsistency to resolve:** [sean2025] says "expected O(log n)" but a uniform-density grid is closer to expected O(1) amortized. Pick one bound and justify it from the density model. |
...
| C-OPT-4 | The all-candidates-intersect case can be handled by reordering hull segments to insert the new edge in O(1). | superseded | proposed in [sean2025] §5; **not** implemented — `legacy/monolithic/seans_concavehull_4.py` uses trimming instead | — | Superseded by C-CHK-1. Do not assert. Mention only as an abandoned earlier idea if useful. |
...
| C-TERM-2 | A global `max_iterations = n³` guard backstops termination in code. | implemented | `legacy/monolithic/seans_concavehull_4.concavehull` | Termination | A safety net is a *smell*: its presence suggests C-TERM-1 isn't yet proven. Either prove C-TERM-1 and frame the guard as defensive, or report it honestly as a current limitation. |
