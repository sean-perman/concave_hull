...
The published algorithm responds to either failure by restarting with $k+1$. Our implementation preserves its rule that the starting point cannot be reintroduced before the fourth vertex. It changes only the terminal behavior: $k=n-1$ is attempted once, and a failed attempt at that cap returns the ordinary convex hull rather than recursively requesting the same capped value again.
\subsection{The Role of the Parameter $k$}
The parameter $k$ is the central control variable in the algorithm. When $k$ is small, candidate selection is highly local, allowing the boundary to follow fine geometric detail and concave features. When $k$ is larger, the set of candidate vertices broadens, and the resulting hull tends to be smoother and closer to a convex enclosure. For $n$ distinct input points, $k=n-1$ is the largest meaningful value because every available point can then be considered. This makes the vertex choice resemble Jarvis march, but does not by itself prove that the complete procedure terminates under the original closure rule.
\begin{figure}[h]
  \centering
...
change the returned hull. The schedule is finite because the all-candidates
value is tested once; if that attempt also fails, the convex-hull fallback
described below is used. Its $O(n\log n)$ cost does not change either restart
bound.
...
The original paper states that repeatedly increasing \(k\) eventually produces a valid hull, but its pseudocode does not establish this for every input. The value of \(k\) is capped at \(n-1\), yet a failed attempt at that value recursively calls the algorithm with \(k+1\), which is immediately reduced back to \(n-1\). The same failed attempt may therefore be repeated indefinitely. Boundedness of \(k\) alone therefore does not imply termination.

Our restart-based implementation uses the following terminal rule. The construction attempts each value in its growth schedule at most once, including a final attempt at \(k=n-1\). If no attempt returns a valid enclosing polygon, it computes the ordinary convex hull with Andrew's monotone-chain algorithm~\cite{andrew1979}.

\paragraph{Proposition.}
For every finite input point set, the modified restart-based algorithm terminates. If the set is non-collinear, it returns a simple polygon containing every input point; if the set is collinear, it terminates with an explicit report that no two-dimensional polygonal hull exists.

\paragraph{Proof.}
Remove duplicate points and let \(n\) be the number that remain. During a fixed-\(k\) attempt, each accepted non-starting vertex is removed from the candidate set. Consequently, after finitely many iterations the attempt either closes its polygon, reaches a dead end, or exhausts its candidates. Under either restart schedule analyzed here, \(k\) strictly increases and never exceeds \(n-1\), which is attempted only once, so the schedule contains finitely many attempts. If one of them produces a valid enclosing polygon, the algorithm stops. Otherwise, the convex-hull fallback terminates. For a non-collinear finite set its result has at least three vertices, is simple, and contains every input point. For a collinear set the convex hull is a segment, which the algorithm detects and reports instead of claiming a polygon. Thus every execution terminates, and every successful execution returns an enclosing polygon. \(\square\)
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.94\textwidth]{../studies/validity_monotonicity/counterexample.png}
...
successful values of $k$. Testing $k=n-1$ once and then using a convex-hull
fallback closes the original termination gap without changing either bound.
...
valid $k$. Together with the terminal fallback, forward geometric growth provides
a finite search schedule, though it may skip smaller successful values of $k$.
% ================================================================
